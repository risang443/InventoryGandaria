...
<div class="p-4 ml-64 bg-[#A7CCED]">
    <div class="mx-10 mb-5 p-8 w-lg bg-[#3a57b4] rounded-md flex justify-center">
        <h1 class="text-2xl text-white font-bold">Pengubahan Data Masuk Stok Barang</h1>
    </div>
    <div class="mx-10 p-8 w-lg bg-[#66a6df] rounded-md">
        <form action="{{ route('input-barang.update', $inputBarang->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="my-3">
                ...
                <select class="block w-full p-2 text-gray-900 border border-gray-300 rounded-b-lg bg-gray-50 text-sm" name="id_barang" id="id_barang" required>
                    ...
                    @foreach($products as $product)
                        <option class="p-2" value="{{ $product->id }}" data-stok="{{ $product->stok }}" {{ $inputBarang->id_barang == $product->id ? 'selected' : '' }}>
                            {{ $product->namabarang }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="my-3">
                ...
                <select class="block w-full p-2 text-gray-900 border border-gray-300 rounded-b-lg bg-gray-50 text-sm" name="suppliers_id" id="suppliers_id" required>
                    ...
                    @foreach($suppliers as $supplier)
                        <option value="{{ $supplier->id }}" {{ $inputBarang->suppliers_id == $supplier->id ? 'selected' : '' }}>
            ...
            <label class="text-sm font-semibold" for="jumlah">Jumlah Barang yang di Masukan :</label>
            <input class="block my-3 w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-sm focus:ring-blue-500 focus:border-blue-500" type="number" name="store" id="store" value="{{ $inputBarang->store }}" required>

            <label class="text-sm font-semibold" for="tanggal_input">Tanggal Input :</label>
            <input class="block my-3 w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-sm focus:ring-blue-500 focus:border-blue-500" type="date" name="tanggal_input" id="tanggal_input" value="{{ $inputBarang->tanggal_input }}" required>
            ...
            @if($inputBarang->fotoInvoiceInput && $inputBarang->fotoInvoiceInput != 'Tidak memiliki Gambar')
                <p>Current Image:</p>
                <img src="{{ asset('storage/' . $inputBarang->fotoInvoiceInput) }}" alt="Invoice Image" width="200">
            @endif

            <label class="text-sm font-semibold" for="keterangan">Keterangan :</label>
            <textarea class="block my-3 h-40 w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-sm focus:ring-blue-500 focus:border-blue-500" name="keterangan" id="keterangan" required>{{ $inputBarang->keterangan }}</textarea>
            ...
            <a href="{{ route('input-barang.index') }}" class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-auto px-5 py-2.5 text-center">Kembali</a>
        </form>
    </div>
</div>
